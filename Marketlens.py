...
def render_trade_setup_card(setup: TradeSetup, current_price: float = None):
    ...
    if current_price:
        ...
        if distance <= AT_RAIL_THRESHOLD:
            distance_text = f"ðŸŽ¯ {distance:.1f} pts away - AT ENTRY"
            at_entry = True
        elif distance <= 15:
            distance_text = f"â³ {distance:.1f} pts away - WATCH"
        else:
            distance_text = f"ðŸ“ {distance:.1f} pts away"
    ...
    if rr_ok and width_ok and confluence_ok and time_ok:
        ...
        decision_reason = f"R:R {setup.rr_ratio_theta_adjusted:.1f}:1 (Î¸-adj), {setup.confluence_strength} rails, Î”â‰ˆ{setup.delta_estimate:.2f}"
    elif not time_ok:
        decision = "â›” TOO LATE"
        ...
        decision = "âŒ NO-GO"
        decision_color = "#ef4444"
        decision_reason = f"R:R only {setup.rr_ratio_theta_adjusted:.1f}:1 after Î¸ (need {MIN_RR_RATIO}:1)"
    elif not width_ok:
        decision = "âŒ NO-GO"
        decision_color = "#ef4444"
    ...
    if setup.theta_period == 'afternoon':
        theta_warning = "âš ï¸ Afternoon - profits reduced ~35% by theta"
    elif setup.theta_period == 'late':
        theta_warning = "ðŸ›‘ Late session - avoid new entries"
    
    # Card header with strike and decision
    if setup.direction == 'CALLS':
        st.success(f"**{emoji} {setup.strike_label}** â€” {decision}")
    else:
        st.error(f"**{emoji} {setup.strike_label}** â€” {decision}")
    ...
    # Decision reason
    if "GO" in decision:
        st.markdown(f"âœ… {decision_reason}")
    elif "NO-GO" in decision or "LATE" in decision:
        st.markdown(f"âŒ {decision_reason}")
    else:
        st.markdown(f"âš ï¸ {decision_reason}")
    ...
    target_data = {
        ...
        "Profit (Î¸-adj)": [f"+${setup.profit_50_theta_adjusted:.0f}", f"+${setup.profit_75 * theta_mult:.0f}", f"+${setup.profit_100 * theta_mult:.0f}"]
    }
    ...
    with col_c:
        st.metric("R:R (Î¸-adj)", f"{setup.rr_ratio_theta_adjusted:.1f}:1")
    with col_d:
    ...
    <div class="trade-panel">
        <div class="trade-panel-header">
            <div style="display: flex; align-items: center; gap: 1rem;">
                <span class="trade-panel-badge {direction_class}">{setup.trade_type}</span>
...
def calculate_contract_expectation(
    ...
    return ContractExpectation(
        ...
        entry_rail=f"{cone_name} {'â–¼' if rail_type == 'descending' else 'â–²'}",
...
def render_action_card(action: ActionCard):
    emoji = 'ðŸŸ¢' if action.direction == 'CALLS' else ('ðŸ”´' if action.direction == 'PUTS' else ('ðŸŸ¡' if action.direction == 'WAIT' else 'â›”'))
...
def render_contract_recommendation(action: ActionCard):
    ...
    <div style="border: 3px solid {border_color}; border-radius: 12px; padding: 1rem; margin-bottom: 1rem; background: linear-gradient(135deg, {border_color}11, {border_color}22);">
        <h2 style="text-align: center; margin: 0; color: {border_color};">ðŸ“‹ {strike_label}</h2>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_entry:
        st.metric("ðŸŽ¯ Entry Level", f"{action.entry_level:.2f}")
    with col_stop:
        st.metric("ðŸ›‘ Stop Loss", f"{action.stop_level:.2f}", delta="-3 pts", delta_color="inverse")
    
    # Exit Strategy
    st.markdown("**ðŸ“Š Exit Strategy (60/20/20):**")
    ...
    with col_width:
        st.metric("ðŸ“ Cone Width", f"{cone_height:.0f} pts")
    with col_rr:
        st.metric("âš–ï¸ R:R @ 50%", f"{ce.risk_reward_50:.1f}:1")
    
    st.caption("ðŸ’¡ Contract moves ~0.33x underlying (strike 15-20 pts OTM)")
    ...
    for cone in cones:
        ...
        <div class="premium-card" style="padding: 1rem;">
            ...
            <div style="display: flex; gap: 1rem;">
                <div style="flex: 1;">
                    <div style="font-size: 0.75rem; color: #64748b;">To â–² {cone.ascending_rail:.2f}</div>
                ...
                <div style="flex: 1;">
                    <div style="font-size: 0.75rem; color: #64748b;">To â–¼ {cone.descending_rail:.2f}</div>
