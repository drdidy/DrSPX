    ...
    with st.sidebar:
        ...
        if st.session_state.vix_anchor_low > 0 and st.session_state.vix_anchor_high > 0:
            ...
            vix_signal = {
                ...
                'data_source': 'MANUAL',
                # VIX Zone levels (0.15 increments)
                'levels_above': [round(vix_resistance + (0.15 * i), 2) for i in range(1, 5)],
                'levels_below': [round(vix_support - (0.15 * i), 2) for i in range(1, 5)],
            }
            
            # Zone size validation
            zone_size = vix_resistance - vix_support
            if 0.13 <= zone_size <= 0.17:
                st.success(f"✓ Valid zone: {zone_size:.2f} (1 level)")
            elif zone_size > 0:
                num_zones = round(zone_size / 0.15)
                st.info(f"Zone spans ~{num_zones} levels ({zone_size:.2f})")
            ...
            for i in range(3, -1, -1):
                level = vix_signal['levels_above'][i]
                ladder_text += f"+{i+1}: {level:.2f} (PUTS extend)\n"
            ladder_text += f"**RES: {vix_resistance:.2f}** ← CALLS Entry\n"
            if vix_support <= vix_current <= vix_resistance:
                ladder_text += f"**NOW: {vix_current:.2f}** ({vix_position_pct:.0f}%)\n"
            ladder_text += f"**SUP: {vix_support:.2f}** ← PUTS Entry\n"
            for i in range(4):
                level = vix_signal['levels_below'][i]
                ladder_text += f"-{i+1}: {level:.2f} (CALLS extend)\n"
