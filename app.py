...
.app-header {
    ...
    padding:2rem; text-align:center; border-radius:0.5rem; margin-bottom:1rem;
    color:#fff;
}
.tab-header {
    font-size:1.2rem; color:#0f3460; font-weight:600; margin-top:1rem;
}
.card {
...
button[name="Generate SPX"] {
    background: #1f4068; color: #fff; border-radius: 0.25rem;
}
button {
...
with tabs[0]:
    ...
    if st.button("🔮 Generate SPX"):
        ...
        for icon, title, price, slope, anchor in [
            ...
            ("⏹️","Close",cp, SLOPES["SPX_CLOSE"],ac),
...
for i, label in enumerate(["TSLA","NVDA","AAPL","AMZN","GOOGL"], start=1):
    with tabs[i]:
        ...
        col = st.columns(2)[0]
        low_p  = col.number_input("🔽 Prev-Day Low Price", value=0.0, format="%.2f", key=f"{label}_lp")
        low_t  = col.time_input("🕒 Prev-Day Low Time", value=datetime(2025,1,1,8,30).time(), step=1800, key=f"{label}_lt")
        high_p = col.number_input("🔼 Prev-Day High Price", value=0.0, format="%.2f", key=f"{label}_hp")
        high_t = col.time_input("🕒 Prev-Day High Time", value=datetime(2025,1,1,8,30).time(), step=1800, key=f"{label}_ht")
