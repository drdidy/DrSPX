...
.sidebar .sidebar-content {
    background:#0b132b; color:#fff; padding:1rem; border-radius:0 0.5rem 0.5rem 0;
}
.app-header {
    background:linear-gradient(90deg,#3a506b,#1f4068);
    padding:2rem; border-radius:0.5rem; margin-bottom:1.5rem; text-align:center; color:#fff;
...
with st.sidebar:
    ...
    forecast_date = st.date_input(
        "Forecast Date", datetime.now().date() + timedelta(days=1)
    )
    st.divider()
    st.subheader("Adjust Slopes")
    for k in SLOPES:
        SLOPES[k] = st.slider(
            k.replace("_", " "), -1.0, 1.0, SLOPES[k], step=0.0001
        )
...
with tabs[0]:
    ...
    hp = c1.number_input(
        "🔼 High Price", value=6185.8, format="%.2f", key="spx_hp"
    )
    ht = c1.time_input(
        "🕒 High Time",
        value=datetime(2025, 1, 1, 11, 30).time(),
        step=1800,
        key="spx_ht",
    )
    cp = c2.number_input(
        "⏹️ Close Price", value=6170.2, format="%.2f", key="spx_cp"
    )
    ct = c2.time_input(
        "🕒 Close Time",
        value=datetime(2025, 1, 1, 15, 0).time(),
        step=1800,
        key="spx_ct",
    )
    lp = c3.number_input(
        "🔽 Low Price", value=6130.4, format="%.2f", key="spx_lp"
    )
    lt = c3.time_input(
        "🕒 Low Time",
        value=datetime(2025, 1, 1, 13, 30).time(),
        step=1800,
        key="spx_lt",
    )
    if st.button("🔮 Generate SPX", key="btn_spx"):
        ...
        for title, price, slope, anchor in [
            ...
            with st.expander(f"Show {title} Anchor Table"):
                ...
                st.dataframe(
                    generate_spx(price, slope, anchor, forecast_date),
...
for i, label in enumerate(["TSLA", "NVDA", "AAPL", "AMZN", "GOOGL"], start=1):
    with tabs[i]:
        st.markdown(
            f'<div class="tab-header">{icons[label]} {label} Forecast</div>',
            unsafe_allow_html=True,
        )
        col = st.columns(2)[0]
        low_p = col.number_input(
            "🔽 Prev-Day Low Price",
            value=0.0,
            format="%.2f",
            key=f"{label}_low_price",
        )
        low_t = col.time_input(
            "🕒 Prev-Day Low Time",
            value=datetime(2025, 1, 1, 8, 30).time(),
            step=1800,
            key=f"{label}_low_time",
        )
        high_p = col.number_input(
            "🔼 Prev-Day High Price",
            value=0.0,
            format="%.2f",
            key=f"{label}_high_price",
        )
        high_t = col.time_input(
            "🕒 Prev-Day High Time",
            value=datetime(2025, 1, 1, 8, 30).time(),
            step=1800,
        ...
        if st.button(f"🔮 Generate {label}", key=f"btn_{label}"):
            ...
            with st.expander("🔻 Low-Anchor Table"):
                st.markdown('<div class="card">', unsafe_allow_html=True)
                st.dataframe(
                    generate_stock(low_p, SLOPES[label], a_low, forecast_date, invert=True),
                    use_container_width=True,
                )
                st.markdown("</div>", unsafe_allow_html=True)
            with st.expander("🔺 High-Anchor Table"):
                st.markdown('<div class="card">', unsafe_allow_html=True)
                st.dataframe(
                    generate_stock(high_p, SLOPES[label], a_high, forecast_date, invert=False),
                    use_container_width=True,
                )
                st.markdown("</div>", unsafe_allow_html=True)
