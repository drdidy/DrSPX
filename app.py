...
with tabs[0]:
    ...
    if is_tue:
        st.markdown("**Tuesday: Contract-Low Entry**")
    ...
    if st.button("🔮 Generate SPX", key="btn_spx"):
        ...
        if is_tue:
            ...
            alt_s=(low2_price-low1_price)/n12
            ...
            for slot in generate_time_blocks():
                ...
                orows.append({"Time":slot,"Projected":round(low1_price+alt_s*b,2)})
            st.dataframe(pd.DataFrame(orows),use_container_width=True)
            bd=datetime.combine(forecast_date - timedelta(days=1),bounce_time)
            bs=SLOPES["SPX_LOW"]
            st.subheader("📈 Thu: Bounce-Low Slope")
            brows=[]
            for slot in generate_time_blocks():
                h,m=map(int,slot.split(":"))
                tgt=datetime.combine(forecast_date,time(h,m))
                b=calculate_spx_blocks(bd,tgt)
                brows.append({"Time":slot,"Projected":round(bounce_price+bs*b,2)})
