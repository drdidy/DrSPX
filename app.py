# Dr Didy SPX Forecast – v1.5.1
# -------------------------------------------
# • META & NFLX coverage
# • colourful gradient anchor cards
# • banner slide-in animation
# • SPX trends 08:30-14:30; others 07:30-14:30
# • Tuesday & Thursday now ALSO show High/Close/Low anchor-trend tables
# • 16:00-17:00 block skipped for SPX
# • Education lives in /pages/1_Playbook.py
...
VERSION  = "1.5.1"
FIXED_CL_SLOPE = -0.5250   # Tuesday fixed line
...
# ── CSS (unchanged, still includes gradient cards & animation) ───────────────
CSS = """<style>
...
.card{flex:1;min-width:220px;padding:1.4rem;border-radius:var(--radius);
      ...
      transition:.25s}.card:hover{transform:translateY(-4px);box-shadow:0 10px 24px rgba(0,0,0,.15)}
.ic{width:3rem;height:3rem;border-radius:var(--radius);display:flex;align-items:center;
...
.high .ic{background:#10b981}.close{background:radial-gradient(circle at 30% 30%,rgba(59,130,246,.25),transparent 70%),rgba(59,130,246,.12)}
.close .ic{background:#3b82f6}.low{background:radial-gradient(circle at 30% 30%,rgba(239,68,68,.25),transparent 70%),rgba(239,68,68,.12)}
.low .ic{background:#ef4444}body.dark .high{background:rgba(16,185,129,.25)}
body.dark .close{background:rgba(59,130,246,.25)}body.dark .low{background:rgba(239,68,68,.25)}
@media(max-width:480px){.banner{padding:.6rem;font-size:1.1rem}.card{min-width:170px;padding:1.1rem}
.ic{width:2.4rem;height:2.4rem;font-size:1.6rem}.val{font-size:1.55rem}body{padding-bottom:80px}}</style>"""
st.markdown(CSS, unsafe_allow_html=True)

def coloured_card(kind,symbol,title,value):
    st.markdown(f"""<div class="card {kind}"><div class="ic">{symbol}</div>
    <div><div class="ttl">{title}</div><div class="val">{value:.2f}</div></div></div>""",
...
share=base64.b64encode(json.dumps(st.session_state.slopes).encode()).decode()
st.sidebar.text_input("🔗 Share-suffix",f"?s={share}",disabled=True)

# ── TIME / BLOCK HELPERS ─────────────────────────────────────────────────────
def slots(start=time(7,30)):
...
SPX_SLOTS=slots(time(8,30)); GEN_SLOTS=slots()
...
def table(price,slope,anchor,fd,slts,spx=True,fan=False):
    rows=[]
    for s in slts:
        h,m=map(int,s.split(":"))
        ...
        rows.append(
            {"Time":s,
             "Entry":round(price+slope*b,2)} if not fan else
...
with tabs[0]:
    ...
    if st.button("Run Forecast"):
        ...
        # ── Tuesday special lines ──
        if day_grp=="Tuesday":
            dt1,dt2=[datetime.combine(fcast_date-timedelta(days=1),t) for t in (cl1_t,cl2_t)]
            alt_slope=(cl2_p-cl1_p)/(blk_spx(dt1,dt2) or 1)
            st.subheader("Contract Line (2-pt)") ; st.dataframe(table(cl1_p,alt_slope,dt1,fcast_date,GEN_SLOTS))
            st.subheader("Fixed SPX Line") ; st.dataframe(table(cl_p,FIXED_CL_SLOPE,
                datetime.combine(fcast_date,cl_t),fcast_date,GEN_SLOTS))
        # ── Thursday special lines ──
        if day_grp=="Thursday":
            dt1,dt2=[datetime.combine(fcast_date-timedelta(days=1),t) for t in (ol1_t,ol2_t)]
            alt_slope=(ol2_p-ol1_p)/(blk_spx(dt1,dt2) or 1)
            st.subheader("Contract Line (2-pt)") ; st.dataframe(table(ol1_p,alt_slope,dt1,fcast_date,GEN_SLOTS))
            st.subheader("Bounce-Low Line") ; st.dataframe(table(b_p,st.session_state.slopes["SPX_LOW"],
                datetime.combine(fcast_date-timedelta(days=1),b_t),fcast_date,GEN_SLOTS))

        # ── Anchor-cards & trends (all five weekdays) ──
        ...
        for lbl,price,key,anchor_dt in [
            ("High",hp,"SPX_HIGH",ah),
            ("Close",cp,"SPX_CLOSE",ac),
            ("Low", lp,"SPX_LOW", al)]:
            st.subheader(f"{lbl} Anchor Trend")
            st.dataframe(table(price,st.session_state.slopes[key],anchor_dt,
...
def stock_tab(idx,tic):
    with tabs[idx]:
        ...
        if st.button("Generate",key=f"go_{tic}"):
            low  = table(lp,st.session_state.slopes[tic],datetime.combine(fcast_date,lt),fcast_date,GEN_SLOTS,False,True)
            high = table(hp,st.session_state.slopes[tic],datetime.combine(fcast_date,ht),fcast_date,GEN_SLOTS,False,True)
