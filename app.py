# Dr Didy SPX Forecast  – v1.4.5
# • larger anchor cards
# • SPX high/close/low trends begin 08:30; other tickers stay 07:30
...
if "init" not in st.session_state:
    st.session_state.update(init=True, theme="Light", slopes=deepcopy(BASE_SLOPES),
                            presets={}, mobile=False, toggles={})
...
.banner{background:linear-gradient(90deg,#0062E6 0%,#33AEFF 100%);color:#fff;
        text-align:center;border-radius:var(--radius);padding:.75rem 1rem;
...
.card{flex:1;min-width:170px;background:var(--card);border-radius:var(--radius);
      padding:1rem;display:flex;align-items:center;box-shadow:var(--shadow)}
.ic{font-size:2rem;margin-right:.6rem}.ttl{font-size:.85rem;opacity:.7}
.val{font-size:1.45rem;font-weight:700}
@media(max-width:480px){
  .banner{padding:.6rem;font-size:1.1rem}
  .card{min-width:140px;padding:.8rem}
  .ic{font-size:1.8rem}.val{font-size:1.25rem}
...
document.addEventListener("load",()=>{
  ...
  if(jump){const el=document.getElementById(jump); if(el) el.scrollIntoView();}
});
</script>
"""
...
with st.sidebar.expander("💾 Presets"):
    pn=st.text_input("Preset name");  save=st.button("Save current")
    if save: st.session_state.presets[pn]=deepcopy(st.session_state.slopes)
...
# ───────────────────── TIME / BLOCK HELPERS ───────────────────────────────────
def slot_list(start=time(7,30)):           # generic slot builder
    base=datetime(2025,1,1,start.hour,start.minute)
    return [(base+timedelta(minutes=30*i)).strftime("%H:%M") for i in range(15-(start.hour==8 and start.minute==30)*2)]

SPX_SLOTS_830 = slot_list(time(8,30))      # 08:30→14:30 (13 rows)
GEN_SLOTS_730 = slot_list()                # 07:30→14:30 (15 rows)

def blk_spx(a,t):
    b=0
    while a<t:
        if a.hour!=16: b+=1       # skips whole 16:00-17:00 maintenance
        a+=timedelta(minutes=30)
    return b
blk_stock=lambda a,t:max(0,int((t-a).total_seconds()//1800))

def line(p,s,anchor,fd,slots,spx=True):
    rows=[]
    for sstr in slots:
...
def fan(p,s,anchor,fd,slots,spx=True):
    rows=[]
    for sstr in slots:
        h,m=map(int,sstr.split(":"))
        tgt=datetime.combine(fd,time(h,m))
        b=blk_spx(anchor,tgt) if spx else blk_stock(anchor,tgt)
        rows.append({"Time":sstr,
                     "Entry":round(p+s*b,2),
                     "Exit": round(p-s*b,2)})
    return pd.DataFrame(rows)
...
def delta_chart(df,col,field="Projected"):
    ...
    return alt.Chart(df.assign(delta=df[field]-base)).mark_line(color=col).encode(
            x="Time",y=alt.Y("delta:Q",title="Δ vs anchor"),tooltip=["Time",field])

def toggle(key,default=False):
    st.session_state.toggles.setdefault(key,default)
    st.session_state.toggles[key]=st.toggle("👁️ Show chart",key=key,value=st.session_state.toggles[key])
    return st.session_state.toggles[key]
...
st.caption("Each **anchor trend** projects price from a chosen prior-day anchor using your slope per 30-min block.")
...
with tabs[0]:
    ...
    if st.button("Run Forecast"):
        ...
        if day_grp=="Tuesday":                       # Tue logic
            dt1,dt2=[datetime.combine(fcast_date-timedelta(days=1),t) for t in (cl1_t,cl2_t)]
            alt_slope=(cl2_p-cl1_p)/(blk_spx(dt1,dt2) or 1)
            df=line(cl1_p,alt_slope,dt1,fcast_date,GEN_SLOTS_730)
            st.subheader("Contract Line (2-pt)"); st.dataframe(df,use_container_width=True)
            if toggle("tue_contract",not st.session_state.mobile):
                st.altair_chart(delta_chart(df,CB[0]),use_container_width=True)
            df2=line(cl_p,FIXED_CL_SLOPE,datetime.combine(fcast_date,cl_t),fcast_date,GEN_SLOTS_730)
            st.subheader("Fixed SPX Line"); st.dataframe(df2,use_container_width=True)
            if toggle("tue_fixed"):
                st.altair_chart(delta_chart(df2,CB[1]),use_container_width=True)

        elif day_grp=="Thursday":                    # Thu logic
            dt1,dt2=[datetime.combine(fcast_date-timedelta(days=1),t) for t in (ol1_t,ol2_t)]
            alt_slope=(ol2_p-ol1_p)/(blk_spx(dt1,dt2) or 1)
            df=line(ol1_p,alt_slope,dt1,fcast_date,GEN_SLOTS_730)
            st.subheader("Contract Line (2-pt)"); st.dataframe(df,use_container_width=True)
            if toggle("thu_contract",not st.session_state.mobile):
                st.altair_chart(delta_chart(df,CB[0]),use_container_width=True)
            df2=line(b_p,st.session_state.slopes["SPX_LOW"],
                     datetime.combine(fcast_date-timedelta(days=1),b_t),fcast_date,GEN_SLOTS_730)
            st.subheader("Bounce-Low Line"); st.dataframe(df2,use_container_width=True)
            if toggle("thu_bounce"):
                st.altair_chart(delta_chart(df2,CB[1]),use_container_width=True)

        else:                                       # Mon/Wed/Fri baseline
            ...
            for txt,val,key,anc,col in [
                ("High",hp,"SPX_HIGH",ah,CB[0]),
                ("Close",cp,"SPX_CLOSE",ac,CB[1]),
                ("Low", lp,"SPX_LOW", al,CB[2])]:
                df=fan(val,st.session_state.slopes[key],anc,fcast_date,SPX_SLOTS_830)
                tag=f"{txt.lower()}_trend"
                st.subheader(f"{txt} Anchor Trend")
                st.dataframe(df,use_container_width=True)
                if toggle(tag):
                    ch=alt.Chart(df.assign(EntryΔ=df["Entry"]-df["Entry"].iloc[0],
                                           ExitΔ =df["Exit"] -df["Entry"].iloc[0])
                                 ).transform_fold(["EntryΔ","ExitΔ"]).mark_line().encode(
...
def stock_tab(idx,tic,col):
    with tabs[idx]:
        ...
        if st.button("Generate",key=f"go_{tic}"):
            df_low =fan(lp,st.session_state.slopes[tic],datetime.combine(fcast_date,lt),fcast_date,GEN_SLOTS_730,False)
            df_hi  =fan(hp,st.session_state.slopes[tic],datetime.combine(fcast_date,ht),fcast_date,GEN_SLOTS_730,False)
            st.dataframe(df_low,use_container_width=True); st.dataframe(df_hi,use_container_width=True)
...
for i,(tic,col) in enumerate(zip(list(ICONS)[1:],CB[1:]),1): stock_tab(i,tic,col)
