...
def blocks_between(t1: datetime, t2: datetime) -> int:
    ...
    blocks, cur = 0, t1
    while cur < t2:
        # mirrors SPX behavior; 16:00 is not within 08:30–14:30 anyway
        if cur.hour != 16:
            blocks += 1
...
def week_mon_tue_fri_for(forecast: date) -> tuple[date, date, date]:
    ...
    # ensure Mon/Tue are strictly before the forecast date
    if tue >= forecast:
...
def compute_weekly_anchors(forecast: date):
    """Pick anchors automatically:
       upper from max(Mon High, Tue High), lower from min(Mon Low, Tue Low)."""
...
with st.sidebar:
    ...
    if colB.button("Next trading day"):
        delta = 1 if date.today().weekday() < 4 else (7 - date.today().weekday())
...
# Summary tiles for Mon/Tue only (professional, compact)
st.markdown(
    f"""
<div class="card">
  <div class="tilegrid">
...
# Channel for selected day
channel_df = channel_for_day(forecast_date, wk)

# Open strength badge for selected day
...
def next_slot_label() -> str:
    ...
    for s in SLOTS:
        if s >= now:
            return s
    return SLOTS[0]  # wrap to first slot
...
def first_touch_close_above(day: date, anchors: dict, tolerance: float):
    """Return first event where bar touches a line AND closes above it.
       Touch means Low<=Line<=High. For upper-line event, Close>=UpperLine;
       for lower-line event, Close>=LowerLine."""
...
def signals_mon_to_fri(forecast: date, anchors: dict, tolerance: float) -> pd.DataFrame:
    mon, _, fri = week_mon_tue_fri_for(forecast)
...
# Color Δ column (green for positive)
def style_delta(s: pd.Series):
    return [("color: #16A34A" if (isinstance(v,(int,float)) and v >= 0) else "color: #DC2626") for v in s]
...
def daily_note() -> str:
    ...
    if open_px is not None:
        lines.append(f"**Open Status:** {textwrap.shorten(badge_html, width=80, placeholder='...')}")
    lines.append("")
